...
const GridView: React.FC<GridViewProps> = ({ data, onAddToCart, onViewDetails }) => {
  ...
  return (
    <div className="responsive-grid responsive-grid--4 auto-fit-grid">
      {data.map((item, index) => (
        <div
          key={item.id}
          className="group relative glass-dark rounded-2xl border border-white/20 p-4 sm:p-6 hover:shadow-2xl smooth-transition hover-lift hover:border-cyan-400/50 grid-item scale-expand"
        ...
        >
          ...
          <div className="text-center">
            {/* Icon */}
            <div className="text-4xl sm:text-5xl lg:text-6xl mb-3 sm:mb-4 group-hover:scale-110 smooth-transition easter-egg-float">
            ...
            <div className="space-y-2 sm:space-y-3">
              <button
                ...
                className="w-full px-3 sm:px-4 py-2 sm:py-3 bg-gradient-to-r from-cyan-500 to-purple-500 hover:from-cyan-600 hover:to-purple-600 text-white rounded-lg smooth-transition font-semibold shadow-lg hover-lift text-sm sm:text-base btn-keyboard-focus ripple-effect"
              >
                🛒 Add to Cart
              </button>
              ...
              <button
                ...
                className="w-full px-3 sm:px-4 py-2 glass-dark hover:bg-white/20 text-white rounded-lg smooth-transition text-xs sm:text-sm btn-keyboard-focus"
              >
                👁️ View Details
              </button>
              
              <button
                onClick={() => setReviewModalOpen({itemId: parseInt(item.id), itemType: item.type, itemTitle: item.title})}
                className="w-full px-3 sm:px-4 py-2 glass-dark hover:bg-white/20 text-white rounded-lg smooth-transition text-xs sm:text-sm btn-keyboard-focus"
              >
                ⭐ Reviews
              </button>
          ...
          {/* Hover Glow Effect */}
          <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-cyan-400/10 to-purple-400/10 opacity-0 group-hover:opacity-100 smooth-transition pointer-events-none" />
        </div>
      ))}
