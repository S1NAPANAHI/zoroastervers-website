-- Migration: Create Character Tables Without Foreign Keys
-- Description: Creates character tables without external foreign key constraints
-- Fix: Temporary version of create_character_management_system, references will be added later
-- Author: System
-- Date: 2025

-- Users Table (needed for created_by foreign key)
...
CREATE TABLE IF NOT EXISTS characters (
    ...
    -- Temporarily removed foreign key constraints
    book_id BIGINT,
    volume_id BIGINT,
    saga_id BIGINT,
    arc_id BIGINT,
...
CREATE INDEX IF NOT EXISTS idx_characters_volume_id ON characters(volume_id);
CREATE INDEX IF NOT EXISTS idx_characters_saga_id ON characters(saga_id);
CREATE INDEX IF NOT EXISTS idx_characters_arc_id ON characters(arc_id);
CREATE INDEX IF NOT EXISTS idx_characters_issue_id ON characters(issue_id);
...
CREATE INDEX IF NOT EXISTS idx_characters_search_vector ON characters USING GIN(search_vector);
...
CREATE INDEX IF NOT EXISTS idx_character_relationships_status ON character_relationships(status);
CREATE INDEX IF NOT EXISTS idx_character_relationships_strength ON character_relationships(strength);
...
CREATE INDEX IF NOT EXISTS idx_character_associations_name ON character_associations USING GIN (to_tsvector('english', association_name));
CREATE INDEX IF NOT EXISTS idx_character_associations_status ON character_associations(status);
...
CREATE INDEX IF NOT EXISTS idx_character_tag_assignments_confidence ON character_tag_assignments(confidence DESC);
...
CREATE TRIGGER IF NOT EXISTS update_character_relationships_updated_at 
    BEFORE UPDATE ON character_relationships 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER IF NOT EXISTS update_character_associations_updated_at 
    BEFORE UPDATE ON character_associations 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER IF NOT EXISTS update_character_tags_updated_at 
...
CREATE TRIGGER IF NOT EXISTS update_character_search_vector_trigger
...
CREATE TRIGGER IF NOT EXISTS update_tag_usage_count_trigger
